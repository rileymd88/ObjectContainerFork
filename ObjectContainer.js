define(["qlik", "css!./ObjectContainer.css"
	...
	function (qlik) {
		...
		var layout = {
			...
			backgroundCustomCss: {
				ref: "backgroundCustomCss",
				label: "Background Custom CSS",
				type: "string",
				defaultValue: "",
				expression: "optional"
			},
		...
		return {
			...
			paint: function ($element, layout) {
				...
				if (layout.backgroundCustomCss) {
					try {
						backgroundCustomCss = JSON.parse(layout.backgroundCustomCss);
						$element.find('.bg').css(backgroundCustomCss);
					}
					catch (err) {
				...
				function renderItems() {
					...
					for (var i = 0; i < layout.listItems.length; i++) {
						...
						if (layout.listItems[i].itemtype == 'subtitle') {
							...
							$element.find('.bg').find('#' + htmlId).css({ 'height': layout.listItems[i].height, 'width': layout.listItems[i].width });
							if (layout.listItems[i].customCss) {
								console.log(layout.listItems[i].customCss);
								try {
									customCssSettings = JSON.parse(layout.listItems[i].customCss);
									$element.find('.bg').find('#' + htmlId).css(customCssSettings);
								}
								catch (err) {
									console.log("error", err);
								}
